...
class TestFormulate(unittest.TestCase):
    ...
    def test_formulatetable_ok(self):
        exp_result = "CREATE TABLE stuff \
            (Command TEXT, Paste TEXT);"
        self.assertEqual(FormulateTable.query(
            "stuff"),
            exp_result)

    def test_formulateinsertdata_ok(self):
        exp_result = "INSERT INTO stuff (Command, Paste) \
            VALUES (\"test\", \"test\");"
        self.assertEqual(FormulateInsertData.query(
            "stuff", "test", "test"),
            exp_result)

    def test_FormulateDelete_ok(self):
        exp_result = "DELETE FROM main_table WHERE Command = \"abc\";"
        self.assertEqual(FormulateDeleteEntry.query(
            "main_table", "abc"),
            exp_result)

    def test_FormulateView_ok(self):
        exp_result = "SELECT * FROM main_table;"
        self.assertEqual(FormulateViewQuery.query(
            "main_table"),
            exp_result)
    ...
    def test_FormulateShowTables(self):
        exp_result = "SELECT name FROM sqlite_master WHERE type='table';"
